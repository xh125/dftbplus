...
contains
  ...
  call errStatus%setError(-1, "Onsite corrected and DFTB+U terms currently not compatible for perturbation",&
      & "/home/xh/packages/DFTB+/dftbplus-23.1/src/dftbp/derivs/perturb.F90", 737)
  return
    end if
    if (allocated(dftbU) .or. allocated(onsMEs)) then
      allocate(dqBlockIn(orb%mOrb,orb%mOrb,nAtom,nSpin))
  ...
  call errStatus%setError(-1, "SCC in perturbation is NOT converged, maximal SCC iterations exceeded",&
      & "/home/xh/packages/DFTB+/dftbplus-23.1/src/dftbp/derivs/perturb.F90", 1487)
  return
      else
