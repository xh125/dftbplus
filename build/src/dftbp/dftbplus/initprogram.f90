...
contains
  ...
  subroutine initProgramVariables(this, input, env)
    ...
    if (allocated(input%ctrl%ppRPA)) then
      ...
      if (this%tSpin) then
        call error("PP-RPA does not support spin polarised ground state")
      end if
      if (this%t3rd) then
        call error("PP-RPA does not support third order")
      end if
      if (any(this%kPoint /= 0.0_dp)) then
        call error("PP-RPA does not support non-gamma k-points")
      end if
      if (this%tFixEf) then
        call error("PP-RPA does not support a fixed Fermi level")
      end if
      if (this%tPoisson) then
        call error("PP-RPA does not support use of the Poisson solver")
      end if
      if (input%ctrl%tShellResolved) then
        call error("PP-RPA does not support shell resolved hamiltonians")
      end if
      ...
      if (allocated(this%solvation)) then
        call error("PP-RPA does not support solvation")
      end if
      if (allocated(this%dftbU)) then
        call error("PP-RPA does not support DFTB+U/pSIC")
      end if
      if (allocated(this%onSiteElements)) then
        call error("PP-RPA does not support onsite corrections")
      end if
      if (allocated(this%reks)) then
        call error("PP-RPA does not support REKS")
      end if
  ...
  subroutine initializeCharges(this, initialSpins, initialCharges)
    ...
    if (.not. allocated(this%reks)) then
      !Input charges packed into unique equivalence elements
        if (.not. allocated(this%qDiffRed)) then
          allocate(this%qDiffRed(this%nMixElements))
        end if
        this%qDiffRed(:) = 0.0_dp
        if (.not. allocated(this%qInpRed)) then
          allocate(this%qInpRed(this%nMixElements))
        end if
        this%qInpRed(:) = 0.0_dp
        if (.not. allocated(this%qOutRed)) then
          allocate(this%qOutRed(this%nMixElements))
        end if
        this%qOutRed(:) = 0.0_dp
    end if
...
block
  type(TTwoBodyRep), allocatable :: inst
  allocate(inst)
  call TTwoBodyRep_init(inst, twoBodyInp)
  call move_alloc(inst, repulsive)
end block
...
block
  type(TRepulsiveCont), allocatable :: inst
  allocate(inst)
  call TRepulsiveCont_init(inst, repulsiveList)
  call move_alloc(inst, repulsive)
end block
    else
