...
99070 format ('Failure in diagonalisation routine zheev,', &
  ...
  !> Single precision eigensolver for generalized symmetric matrix problem
  subroutine real_ssygv(a, b, w, uplo, jobz, itype)
    ...
    call ssygv(iitype, jobz, uplo, n, a, lda, b, ldb, w, idealwork, -1, info)
    if (info/=0) then
       call error("Failure in ssygv to determine optimum workspace")
    endif
    int_idealwork=nint(idealwork(1))
    allocate(work(int_idealwork))
    call ssygv(iitype, jobz, uplo, n, a, lda, b, ldb, w, work, int_idealwork, info)
    if (info/=0) then
      if (info<0) then
        write(error_string, "('Failure in diagonalisation routine ssygv, illegal ',&
            & 'argument at position ',i6)") info
        call error(error_string)
      else if (info <= n) then
        write(error_string, "('Failure in diagonalisation routine ssygv, diagonal ',&
            & 'element ', i6, ' did not converge to zero.')") info
        call error(error_string)
      else
        write(error_string, "('Failure in diagonalisation routine ssygv,', &
            & ' non-positive definite overlap! Minor ',i6,' responsible.')") info - n
        call error(error_string)
      endif
    endif
...
99550 format ('Failure in diagonalisation routine zhbgv,', &
  ...
  subroutine real_sgeev(a, wr, wi, vl, vr, err)
    ...
    if (jobvl == 'V' .and. jobvr == 'V') then
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, vr, ldvr, idealwork, -1, info)
    else if (jobvl == 'V' .and. jobvr == 'N') then
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, dummyvr, ldvr, idealwork, -1,&
          & info)
    else if (jobvl == 'N' .and. jobvr == 'V') then
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, vr, ldvr, idealwork, -1,&
          & info)
    else if (jobvl == 'N' .and. jobvr == 'N') then
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, dummyvr, ldvr, idealwork,&
          & -1, info)
    end if
    if (info/=0) then
  block
    ...
    write(stringTmp,"(A)")"Failue in sgeev to determine optimum workspace"
    if (present(err)) then
      err = -1
    ...
    if (jobvl == 'V' .and. jobvr == 'V') then
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, vr, ldvr, work, int_idealwork,&
          & info)
    else if (jobvl == 'V' .and. jobvr == 'N') then
      ...
      call sgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, dummyvr, ldvr, work,&
          & int_idealwork, info)
    end if
  ...
  block
    ...
    write(stringTmp,"(A,I0)") 'Failure in diagonalisation routine sgeev, illegal argument at position ',info
    if (present(err)) then
      err = info
      call warning(stringTmp)
      return
  ...
  block
    ...
    write(stringTmp,"(A,I0,A)") 'Failure in diagonalisation routine sgeev, diagonal element ',info,' did not converge to zero.'
    if (present(err)) then
      err = info
      call warning(stringTmp)
      return
  ...
  !> Simple general matrix eigensolver
  subroutine dble_dgeev(a, wr, wi, vl, vr, err)
    ...
    if (jobvl == 'V' .and. jobvr == 'V') then
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, vr, ldvr, idealwork, -1, info)
    else if (jobvl == 'V' .and. jobvr == 'N') then
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, dummyvr, ldvr, idealwork, -1,&
          & info)
    else if (jobvl == 'N' .and. jobvr == 'V') then
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, vr, ldvr, idealwork, -1,&
          & info)
    else if (jobvl == 'N' .and. jobvr == 'N') then
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, dummyvr, ldvr, idealwork,&
          & -1, info)
    end if
    if (info/=0) then
  block
    ...
    write(stringTmp,"(A)")"Failue in dgeev to determine optimum workspace"
    if (present(err)) then
      err = -1
      call warning(stringTmp)
      return
    ...
    if (jobvl == 'V' .and. jobvr == 'V') then
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, vl, ldvl, vr, ldvr, work, int_idealwork,&
          & info)
    else if (jobvl == 'V' .and. jobvr == 'N') then
      ...
      call dgeev(jobvl, jobvr, n, a, lda, wr, wi, dummyvl, ldvl, dummyvr, ldvr, work,&
          & int_idealwork, info)
    end if
  ...
  block
    ...
    write(stringTmp,"(A,I0)") 'Failure in diagonalisation routine dgeev, illegal argument at position ',info
    if (present(err)) then
      err = info
      call warning(stringTmp)
      return
  ...
  block
    ...
    write(stringTmp,"(A,I0,A)") 'Failure in diagonalisation routine dgeev, diagonal element ',info,' did not converge to zero.'
    if (present(err)) then
      err = info
      call warning(stringTmp)
      return
