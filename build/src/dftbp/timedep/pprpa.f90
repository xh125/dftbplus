...
contains
  ...
  block
    ...
    write(stringTmp,"(A)")"spin-unrestricted calculations currently not possible with pp-RPA"
    if (present(err)) then
      err = -1
  ...
  block
    ...
    write(stringTmp,"(A)")"Fractionally charged systems not possible with pp-RPA"
    if (present(err)) then
      err = -1
      call warning(stringTmp)
      return
  ...
  block
    ...
    write(stringTmp,"(A)")"Odd numbers of electrons not possible with pp-RPA"
    if (present(err)) then
      err = -1
      call warning(stringTmp)
      return
  ...
  block
    ...
    write(stringTmp,"(A,I0)") " Error with dgeev, info = ",info
    if (present(err)) then
      err = info
      call warning(stringTmp)
      return
